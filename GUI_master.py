...
#    id = input('enter user id')
    ...
    while 1:
        ...
        for (x,y,w,h) in faces:
            ...
            cv2.waitKey(100)#allows users to display a window for given milliseconds or until any key is pressed.
    
        cv2.imshow('img',img)#To be able to display an image by creating a window,
    ...
    cap.release()
    entry2.delete(0,'end')#specify the value of that element and pass it as an argument to the method.
...
def Train_database():
    ...
    def getImagesWithID(path):
        ...
        imagePaths = [os.path.join(path, f) for f in os.listdir(path)] #Multiples path Combine then read  to face data store  
